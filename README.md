...
WebUI 三栏布局：左侧输入面板（上传 + 描述）+ 中间 WebGL shader 实时预览 + 右侧 pipeline 进度日志 / 参数 / 评分面板。codex agent 在后台自主完成 6-phase 工作流，前端实时推送状态。
...
**当前 MVP 后端**：[codex CLI](https://developers.openai.com/codex)（OpenAI）。
**未来扩展**：参照 AutoResearchClaw 的 acpx 协议思路，未来可扩展到 [Claude Code](https://docs.anthropic.com/en/docs/claude-code)、[OpenCode](https://opencode.ai) 等支持 agent skills 标准的后端 — orchestrator 层无需改动，只需替换 codex CLI 调用。
