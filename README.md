...
| Agent | Role (职业定位) | 输入 | 输出 | 职责 |
|-------|----------------|------|------|------|
| **Decompose** | 视觉分析专家 | 关键帧图片 + 视频元信息 | visual_description (JSON) | 分析视觉元素，提取量化参数（RGB、时长、比例） |
| **Generate** | 图形程序开发专家 | visual_description + Skill | GLSL Shader (Shadertoy 格式) | 理解语义意图，编写 Mobile GPU 优化 Shader |
| **Inspect** | 视效技术总监 | 渲染截图 + 设计参考截图 | 对比评分 + 语义反馈 | 多维度量化评估，输出可操作修正建议 |

#### 技术特性

- **System Prompt 架构**（参考 Google GenerativeUI）：Role → Philosophy → Common Info → Planning Instructions → Skill → Failure Examples
- **真实职业角色定位**：视觉分析专家 / 图形程序开发专家 / 视效技术总监（而非虚构角色）
- **共享公共信息**：三 Agent 共享平台、协作规则、视觉标准、术语约定
